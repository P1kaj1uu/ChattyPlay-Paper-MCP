...
import { VERSION, canonicalPaperId, downloadPaper, getPaper, getRelatedResources, isValidPaperId, readPaper, searchPapers } from './papers.js'
...
function createServer() {
  ...
  server.registerTool('search_papers', {
    ...
    try {
      if ([input.date, input.week, input.month].filter(Boolean).length > 1) throw new Error('date、week、month 只能选择一个')
      return result(await searchPapers(input))
  ...
  server.registerTool('get_paper', {
    ...
    inputSchema: { id: paperId },
    outputSchema: paperSchema.shape,
    annotations: { readOnlyHint: true, openWorldHint: true }
  }, async ({ id }) => {
  ...
  server.registerTool('get_related_resources', {
    ...
    inputSchema: { id: paperId, limit: z.number().int().min(1).max(50).default(10) },
    outputSchema: {
